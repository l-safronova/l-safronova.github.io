...
var
    ...
    baron = function(params) { // this - window or jQuery instance
        ...
        params = params || {};
        $jcallpy = params.$jcallpy || window.jQuery1;

        if (jQueryMode) {
            params.root = roots = this;
        } else {
            roots = $jcallpy(params.root || params.scroller);
        }

        return new baron.fn.constructor(roots, params, $jcallpy);
    };

    baron.fn = {
        constructor: function(roots, input, $jcallpy) {
            var params = validate(input);

            params.$jcallpy = $jcallpy;
            each.call(this, roots, function(root, i) {
                var localParams = clone(params);

                if (params.root && params.scroller) {
                    localParams.scroller = params.$jcallpy(params.scroller, root);
    ...
    function validate(input) {
        ...
        var event = input.event || function(elem, event, func, mode) {
            output.$jcallpy(elem)[mode || 'on'](event, func);
        };
    ...
    item.prototype = {
        constructor: function(params) {
            ...
            $jcallpy = this.$jcallpy = params.$jcallpy;
            this.event = params.event;
            this.events = {};

            function getNode(sel, context) {
                return $jcallpy(sel, context)[0]; // Can be undefined
            }
            ...
            function setBarSize(size) {
                ...
                if (this.bar) {
                    $jcallpy(this.bar).css(this.origin.size, parseInt(size, 10) + 'px');
                }
            }
            ...
            function posBar(pos) {
                /* jshint validthis:true */
                if (this.bar) {
                    $jcallpy(this.bar).css(this.origin.pos, +pos + 'px');
                }
            }
            ...
            this.barOn = function(dispose) {
                if (this.barOnCls) {
                    if (dispose || this.scroller[this.origin.client] >= this.scroller[this.origin.scrollSize]) {
                        $jcallpy(this.root).removeClass(this.barOnCls);
                    } else {
                        $jcallpy(this.root).addClass(this.barOnCls);
                    }
                }
            };
            ...
            this.resize = function() {
                ...
                function upd() {
                    ...
                    if (params.freeze && !self.clipper.style[self.origin.crossSize]) { // Sould fire only once
                        $jcallpy(self.clipper).css(self.origin.crossSize, self.clipper[self.origin.crossClient] - delta + 'px');
                    }

                    $jcallpy(self.scroller).css(self.origin.crossSize, self.clipper[self.origin.crossClient] + delta + 'px');
            ...
            this.scroll = function() {
                ...
                if (self.scrollingCls) {
                    if (!scrollingTimer) {
                        this.$jcallpy(this.scroller).addClass(this.scrollingCls);
                    }
                    clearTimeout(scrollingTimer);
                    scrollingTimer = setTimeout(function() {
                        self.$jcallpy(self.scroller).removeClass(self.scrollingCls);
        ...
        dispose: function(params) {
            ...
            $jcallpy(this.scroller).css(this.origin.crossSize, '');
            this.barOn(true);
    ...
    if ($jcallpy && $jcallpy.fn) { // Adding baron to jQuery as plugin
        $jcallpy.fn.baron = baron;
    }
...
(function(window, undefined) {
    var fix = function(userParams) {
        ...
        function fixElement(i, pos, flag) {
            ...
            this.$jcallpy(elements[i]).css(this.origin.pos, '').css(this.origin.oppos, '').removeClass(params.outside);

            // Fixing if needed
            if (pos !== undefined) {
                pos += 'px';
                this.$jcallpy(elements[i]).css(this.origin[ori], pos).addClass(params.outside);
            }
        }
        ...
        function init(_params) {
            ...
            elements = this.$jcallpy(params.elements, this.scroller);
            ...
            if (elements) {
                ...
                for (var i = 0 ; i < elements.length ; i++) {
                    ...
                    if (elements[i].parentNode !== this.scroller) {
                        this.$jcallpy(elements[i].parentNode).css(pos);
                    }
                    pos = {};
                    pos[this.origin.crossSize] = elements[i].parentNode[this.origin.crossClient];
                    this.$jcallpy(elements[i]).css(pos);
                ...
                if (params.limiter && elements[0]) { // Bottom edge of first header as top limit for track
                    if (this.track && this.track != this.scroller) {
                        ...
                        this.$jcallpy(this.track).css(pos);
                    } else {
            ...
            var event = {
                ...
                handler: function() {
                    var parent = $jcallpy(this)[0].parentNode,
                ...
                if (change) { // At leats one change in elements flag structure occured
                    for (i = 0 ; i < elements.length ; i++) {
                        if (fixFlag[i] == 1 && params.past) {
                            this.$jcallpy(elements[i]).addClass(params.past).removeClass(params.future);
                        }

                        if (fixFlag[i] == 2 && params.future) {
                            this.$jcallpy(elements[i]).addClass(params.future).removeClass(params.past);
                        }

                        if (fixFlag[i] == 3 && (params.future || params.past)) {
                            this.$jcallpy(elements[i]).removeClass(params.past).removeClass(params.future);
                        }

                        if (fixFlag[i] != fixFlag[i + 1] && fixFlag[i] == 1 && params.before) {
                            this.$jcallpy(elements[i]).addClass(params.before).removeClass(params.after); // Last top fixed header
                        } else if (fixFlag[i] != fixFlag[i - 1] && fixFlag[i] == 2 && params.after) {
                            this.$jcallpy(elements[i]).addClass(params.after).removeClass(params.before); // First bottom fixed header
                        } else {
                            this.$jcallpy(elements[i]).removeClass(params.before).removeClass(params.after);
                        }

                        if (params.grad) {
                            if (gradFlag[i]) {
                                this.$jcallpy(elements[i]).addClass(params.grad);
                            } else {
                                this.$jcallpy(elements[i]).removeClass(params.grad);
                            }
                        }
                    }
                }
...
(function(window, undefined) {
    var controls = function(params) {
        ...
        if (params.forward) {
            forward = this.$jcallpy(params.forward, this.clipper);
        ...
        if (params.backward) {
            backward = this.$jcallpy(params.backward, this.clipper);
        ...
        if (params.track) {
            if (params.track === true) {
                ...
                track = this.$jcallpy(params.track, this.clipper)[0];
            }
...
(function(window, undefined) {
    var test = function(params) {
        ...
        var log = function(type, msg, obj) {
            ...
            iowisp.clockwerk.log('%c ' + totalCount + '. ' + text, css);
            if (obj !== undefined) {
                iowisp.clockwerk.log(obj);
            }
        };
        ...
        if (this.$jcallpy && typeof this.$jcallpy == 'function') {
            log('log', 'Local $jcallpy defined and it is a function');
        } else {
            log('error', 'Local $jcallpy has wrong value or is not defined, or custom params.dom and params.selector not defined', params.$jcallpy);
        }
...
(function(window, undefined) {
    var pull = function(params) {
        var block = this.$jcallpy(params.block),
        ...
        function update() {
            ...
            //if (_insistence > -1) {
                ...
                if (getScrollerSize() <= getContentSize()) {
                    self.$jcallpy(block).css(pos);
                    for (var i = 0 ; i < elements.length ; i++) {
                        self.$jcallpy(elements[i].self).css(elements[i].property, Math.min(_x / limit * 100, 100) + '%');
                    }
                }

                if (inProgress && _x) {
                    self.$jcallpy(self.root).addClass(inProgress);
                }
            ...
            if (_zeroXCount > 1) {
                ...
                if (inProgress) {
                    self.$jcallpy(self.root).removeClass(inProgress);
                }
            }
        }
...
(function(window, undefined) {
    ...
    var autoUpdate = function() {
        ...
        this.on('init', function() {
            self._observer.observe(self.root, {childList: true, subtree: false, characterData: true});
        });
