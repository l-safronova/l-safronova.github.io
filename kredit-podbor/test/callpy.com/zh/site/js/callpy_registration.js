...
$(document).ready(function(){
    ...
    $(document.body).on('submit', '#sign-up-form', function() {
        ...
        if (check_email(login_data.email.target) == true) {
            ...
            login_data.email.target.callpy_error("e-mail格式不正确").on('input.callpy_error', function() {
        ...
        if (login_data.password.target.val() !== '') {
            ...
            login_data.password.target.callpy_error("请输入密码").on('input.callpy_error', function() {
        ...
        if (login_data.email.val !== false && login_data.password.val !== false) {
            $.get("/clpy/", {
                ...
                if (data["answer"] == "false") {
                    ...
                    login_data.email.target.callpy_error("这样的用户名/密码不存在");
        ...
        if (check_email(input)) {
            ...
            $.post('/clpy/', {
                ...
                if (!data.answer.result) {
                    input.callpy_error("这个e-mail已经在系统中注册！");
            ...
            input.callpy_error("e-mail格式不正确");
            input.on('input.callpy_error', function() {
                input = $(this);
                if (check_email(input)) {
                    input.callpy_error('off').off('.error');
