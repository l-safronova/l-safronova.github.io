...
$(document).ready(function(){
    ...
    $(document.body).on('submit', '#password-recovery-form', function() {
        ...
        if (check_email(email) == false) {
            ...
            $.post('/manager/restore/', {
                ...
                if (data == 'false') {
                    email.callpy_error("Email incorrect");
    ...
    $(document.body).on('submit', '#sign-up-form', function() {
        ...
        if (check_email(login_data.email.target) == true) {
            ...
            login_data.email.target.callpy_error("Format de courriel non valide").on('input.callpy_error', function() {
        ...
        if (login_data.password.target.val() !== '') {
            ...
            login_data.password.target.callpy_error("Entrez le mot de passe").on('input.callpy_error', function() {
        ...
        if (login_data.email.val !== false && login_data.password.val !== false) {
            $.get("/clpy/", {
                ...
                if (data["answer"] == "false") {
                    ...
                    login_data.email.target.callpy_error("Cette paire de login/mot de passe n'existe pas");
        ...
        if (check_email(input)) {
            ...
            $.post('/clpy/', {
                ...
                if (!data.answer.result) {
                    input.callpy_error("Cette adresse email est déjà enregistrée dans le système !");
            ...
            input.callpy_error("Format de courriel non valide!");
            input.on('input.callpy_error', function() {
                input = $(this);
                if (check_email(input)) {
                    input.callpy_error('off').off('.error');
    ...
    $('.test-drive-button').on('click', function(){
        ...
        if (/^[a-zа-яё0-9_-]+(\.[a-zа-яё0-9_-]+)*\.[a-zа-яё]{2,6}$/i.test(input.val())) {
            ...
            input.callpy_error("Format incorrect!");
            input.on('input.callpy_error', function() {
                input = $(this);
                if (check_email(input)) {
                    input.callpy_error('off').off('.error');
